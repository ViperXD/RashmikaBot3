...
async def inline_id_handler(event: events.InlineQuery.Event):
    ...
    if ";" in k:
         testinput,rashmika = event.pattern_match.group(1).split(";")
    else:
    ...
    if lund == lund:
        results = []
        search = SearchVideos(f"{testinput}", offset=1, mode="dict", max_results=int(rashmika))
        mi = search.result()
...
__help__ = """
 - `@midukkirashmika3_bot yt <search your query> ;8`
